...
public class MyInflater : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i_tri = 0; i_tri < tri2vtx.Length / 3; ++i_tri)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                int vtx_id = node2vtx[i];
                float3 gv = grad_volume[i];
                rhs_vector[vtx_id] += lambda * gv;

                bsm.AddBlockAt(vtx_id, num_vtx, new float3x3(
                    new float3(gv.x, 0, 0),
                    new float3(gv.y, 0, 0),
                    new float3(gv.z, 0, 0)
                ));
                bsm.AddBlockAt(num_vtx, vtx_id, new float3x3(
                    new float3(gv.x, gv.y, gv.z),
                    new float3(0, 0, 0),
                    new float3(0, 0, 0)
                ));
            }
            rhs_vector[num_vtx] -= volume;
            // end of the edit
            // ---------------------
        }
...
//        Debug.Assert(false);
    ...
    static (float, float3[]) volume_tri_origin_graident(float3[] node2xyz)
    {
        ...
        gradient[0] = math.cross(node2xyz[1], node2xyz[2]) / 6.0f;
        gradient[1] = math.cross(node2xyz[2], node2xyz[0]) / 6.0f;
        gradient[2] = math.cross(node2xyz[0], node2xyz[1]) / 6.0f;
        // end of editing
        // ---------------
